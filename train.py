...
def parse_args():
    ...
    parser.add_argument('--arch', '-a', metavar='ARCH', default='UNext', choices=ARCH_NAMES, help='model architecture: ' + ' | '.join(ARCH_NAMES))
    ...
    parser.add_argument('--loss', default='BCEDiceLoss', choices=LOSS_NAMES, help='loss function: ' + ' | '.join(LOSS_NAMES))
    ...
    parser.add_argument('--optimizer', default='Adam', choices=['Adam', 'SGD'], help='loss function: ' + ' | '.join(['Adam', 'SGD']))
    parser.add_argument('--lr', '--learning-rate', default=1e-3, type=float, metavar='LR', help='initial learning rate')
    parser.add_argument('--momentum', default=0.9, type=float, metavar='M', help='momentum')
    parser.add_argument('--weight_decay', default=1e-4, type=float, metavar='W', help='weight decay')
    parser.add_argument('--nesterov', default=False, type=str2bool, help='nesterov momentum')
    ...
    parser.add_argument('--early_stopping', default=-1, type=int, metavar='N', help='early stopping epochs (default: -1)')
    parser.add_argument('--cfg', type=str, default=None, help='load config from cfg file')
...
def train(config, train_loader, model, criterion, optimizer):
    ...
    for input, target, _ in train_loader:
        ...
        if config['deep_supervision']:
            ...
            loss /= len(outputs)
            iou = iou_score(outputs[-1], target)[0]  # Corregido: toma solo el valor numérico
        else:
            output = model(input)
            loss = criterion(output, target)
            iou = iou_score(output, target)[0]      # Corregido: toma solo el valor numérico
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def main():
    ...
    # Transformaciones con Albumentations corregidas
    ...
    train_img_ids = glob(os.path.join('inputs', config['dataset'], 'images', '*' + config['img_ext']))
    train_img_ids = [os.path.splitext(os.path.basename(p))[0] for p in train_img_ids]
    
    split = int(len(train_img_ids) * 0.8)
    val_img_ids = train_img_ids[split:]
    ...
    if config['optimizer'] == 'Adam':
        ...
        optimizer = optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=config['lr'], momentum=config['momentum'], weight_decay=config['weight_decay'], nesterov=config['nesterov'])
